    ...
    <div className="h-full flex overflow-hidden bg-neutral-950">
        ...
        <div className="flex-1 p-3 space-y-2.5 overflow-hidden">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="flex items-start gap-4 p-4 rounded-xl">
              <Skeleton className="size-16 rounded-full bg-neutral-800 shrink-0" />
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 p-6 space-y-5 overflow-hidden">
          {[...Array(6)].map((_, i) => (
            <div key={i} className={`flex gap-4 ${i % 2 === 0 ? 'justify-start' : 'justify-end'}`}>
              {i % 2 === 0 && <Skeleton className="size-12 rounded-full bg-neutral-800 shrink-0" />}
              <div className={`space-y-2 ${i % 2 === 0 ? '' : 'flex flex-col items-end'}`}>
                <Skeleton className={`h-24 ${i % 2 === 0 ? 'w-80' : 'w-72'} rounded-2xl bg-neutral-800`} />
        ...
        <div className="p-6 border-t border-neutral-800 bg-neutral-900/30">
          <div className="flex items-center gap-4">
            <Skeleton className="flex-1 h-18 rounded-lg bg-neutral-800" />
            <Skeleton className="size-16 rounded-full bg-neutral-800" />
          </div>
        </div>
      </div>
