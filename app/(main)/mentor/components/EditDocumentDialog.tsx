...
export function EditDocumentDialog({
  ...
  const [documentType, setDocumentType] = useState<MentorDocumentType>(
    document.type
  );
  ...
  const [issuedYear, setIssuedYear] = useState<number | undefined>(
    document.issuedYear || undefined
  );
  ...
  const handleUpdate = async () => {
    try {
      ...
      await mentorService.updateDocument(
        applicationId,
        document.id,
        updateData
      );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-xl p-0">
