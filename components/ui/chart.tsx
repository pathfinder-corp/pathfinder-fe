...
const ChartContainer = React.forwardRef<
  ...
  React.HTMLAttributes<HTMLDivElement> & {
    config: ChartConfig;
    children: React.ComponentProps<typeof TooltipProvider>['children'];
  }
...
const ChartTooltipContent = ({
  ...
  return (
    <div
      className={`rounded-lg border border-neutral-700 bg-neutral-800 px-5 py-3 shadow-lg ${className || ''}`}
    >
      {tooltipLabel && (
        <div className="mb-1.5 text-base font-medium text-neutral-300">
          {tooltipLabel}
        </div>
      )}
            ...
            >
              {!hideIndicator && (
                ...
                >
                  ...
                  {indicator === 'line' && (
                    <div
                      className="size-0.5 w-3 bg-current"
                      style={{ backgroundColor: indicatorColor }}
                    />
                  )}
                  {indicator === 'dashed' && (
                    <div
                      className="size-0.5 w-3 border border-dashed bg-transparent"
