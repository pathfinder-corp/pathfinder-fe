...
export function TimeframePicker({ value, onChange, className, placeholder = 'Enter amount' }: TimeframePickerProps) {
  ...
  const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (inputValue === '' || /^\d+$/.test(inputValue)) {
      ...
      if (numericValue !== undefined && (numericValue < 1 || numericValue > 999)) {
        return;
      }
  ...
  return (
    <div className={cn('flex gap-2', className)}>
      ...
      <Select value={unit} onValueChange={handleUnitChange}>
        <SelectTrigger className="w-45 h-20! text-xl! px-6!">
          <SelectValue>
            {getUnitLabel(unit, amount === '' ? undefined : parseInt(amount, 10))}
          </SelectValue>
        </SelectTrigger>
        <SelectContent>
          {TIME_UNITS.map(timeUnit => (
            <SelectItem key={timeUnit.value} value={timeUnit.value} className="text-xl!">
              {getUnitLabel(timeUnit.value, amount === '' ? undefined : parseInt(amount, 10))}
            </SelectItem>
          ))}
        </SelectContent>
      </Select>
